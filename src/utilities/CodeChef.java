...
public class CodeChef {
    ...
    */
     ...
     * Chef has N coins; let's denote the value of coin i by vi​. 
     * Since today is Eid, Chef is going to give one coin to each of his two children. 
     * He wants the absolute value of the difference between the values of coins given to the two children to be as small as possible, 
     * so that he would be as fair as possible.
     ...
     * 1 <= T <= 100
     * 2 <= N <= 10^5
     * the sum of N in all test cases does not exceed 5 * 10^5
     * 1 <= vi <= 10^6 for each valid i
     */
    public static void solveCodeChefEID() {
        Scanner scn = new Scanner(System.in);
        int T = scn.nextInt(); 
        while (T-- > 0) {
            long N = scn.nextLong();
            ...
            for (long i = 0; i < N; i++) {
                long Vi = scn.nextLong();
                Long floor = tree.floor(Vi);
                Long ceiling = tree.ceiling(Vi);
                if (floor != null)
                    minDiff = Math.min(minDiff, Vi - floor);
                if (ceiling != null)
                    minDiff = Math.min(minDiff, ceiling - Vi);
                tree.add(Vi);
            }
     ...
     * Input Format:
     * The only line of input will contain two space-separated integers X and Y - 
     * the number of questions already answered by Chef, and the number of questions in each worksheet.
     * 
     * Output Format:
     ...
     * 1 <= Y <= 10
     */
    public static void solveCodeChefHWFIN() {
        Scanner scn = new Scanner(System.in);
        long X = scn.nextLong(); 
        long Y = scn.nextLong(); 
     ...
     * Chef moves one step forward each second (that is he moves to X1 ​+ 1 after the 1st second, X1 + 2 after the 2nd2nd second, and so on), 
     ...
     * You need to determine if Chef will be able to meet his friend or not.
     * You can assume that Chef and his friend both keep on moving for a long indefinite amount of time and also that they move simultaneously.
     * 
     * Input Format:
     ...
     * 1 <= X1, X2 <= 10^9
     */
    public static void solveCodeChefFRIMEET() {
        Scanner scn = new Scanner(System.in);
        long T = scn.nextLong(); 
        while (T-- > 0) {
            long X1 = scn.nextLong(); 
            long X2 = scn.nextLong(); 
            System.out.println((X1 - X2) >= 0 ? "YES" : "NO");
        }
        scn.close();
    }    
     ...
     * Output Format:
     * For each test case, output on a new line, 
     * YES if there exists a sequence of moves such that the strings A and B are same at the end of the game, and NO otherwise.
     * You may print each character of the string in uppercase or lowercase 
     * (for example, the strings YES, yEs, yes, and yeS will all be treated as identical).
     * 
     * Constraints:
     ...
     * The sum of N over all test cases does not exceed 2*10^5.
     */
    public static void solveCodeChefABSTRING() {
        Scanner scn = new Scanner(System.in);
        long T = scn.nextLong();
        while (T-- > 0) {
            long N = scn.nextLong();
            ...
            HashMap<Character, Long> hashMap = new HashMap<>();
            while (N-- > 0) {
                if (hashMap.getOrDefault(S.charAt((int)N), 0L) == 0)
                    hashMap.put(S.charAt((int)N), 1L);
                else
                    hashMap.remove(S.charAt((int)N));
            }
            System.out.println(hashMap.size() == 0 ? "YES" : "NO");
        }
        scn.close();
    }
     ...
     * Problem: Path Parity
     * 
     * You are given an integer N. Let A be an N x N grid such that Ai,j = i + N * (j − 1) for 1 <= i, j <= N. 
     * For example, if N = 4 the grid looks like:
     * 
     * 1 5 9 13
     * 2 6 10 14
     * 3 7 11 15
     * 4 8 12 16
     * 
     * You start at the top left corner of the grid, i.e, cell (1, 1). 
     * You would like to reach the bottom-right corner, cell (N, N). 
     * To do so, whenever you are at cell (i, j), you can move to either cell (i + 1, j) or cell (i, j + 1) provided that the corresponding cell lies within the grid 
     * (more informally, you can make one step down or one step right).
     ...
     * K ∈ {0, 1}
     */
    public static void solveCodeChefPATHPAR() {
        Scanner scn = new Scanner(System.in);
        long T = scn.nextLong();
        while (T-- > 0) {
            long N = scn.nextLong();
     ...
     * Chef has a hand with N fingers (for the sake of simplicity, let us consider the thumb a finger too) numbered 1 through N 
     * and the glove has N sheaths numbered 1 through N. Exactly one finger has to be inserted into each sheath. 
     ...
     * Chef can wear the glove normally (the front side of the glove matching the front of Chef's hand), 
     * or she can flip the glove and then insert her hand in it. In the first case, the first finger will go into the first sheath, 
     * the second finger into the second one, and so on. 
     * However, when she flips the glove, her first finger will go into the N-th sheath, 
     * the second finger into the (N - 1)-th and so on — the i-th finger will go into the (N + 1 - i)-th sheath for each valid i. 
     ...
     * If she can only wear the glove in the former way, output "front"; 
     * if she can wear it only in the latter way, output "back". 
     ...
     */
    public static void solveCodeChefCHEGLOVE() {
        Scanner scn = new Scanner(System.in);
        long T = scn.nextLong();
        while (T-- > 0) {
            long N = scn.nextLong();
            ...
            boolean back = true;
            for (long i = 0; i < N; i++) {
                long Gi = scn.nextLong();
                if (Fi.get((int)i) > Gi)
                    front = false;
                if (Fi.get((int)N - 1 - (int)i) > Gi)
                    back = false;
            }
            if (front && back)
                System.out.println("both");
     ...
     * while the second one chooses a random box with the smallest number of apples and puts K apples in it.
     ...
     * Therefore, two distributions of apples are different if there is a box such that the number of apples in it when the first candidate finishes working 
     * can be different from the number of apples in it when the second candidate finishes working.
     * 
     * Input:
     * The first line of the input contains a single integer T denoting the number of test cases. The description of T test cases follows.
     * The first and only line of each test case contains two space-separated integers N and K.
     * 
     * Output:
     * For each test case, print a single line containing the string "YES" if the final distributions of apples can be different 
     * or "NO" if they will be the same (without quotes).
     * 
     * Constraints:
     * 1 <= T <= 250
     * 1 <= N, K <= 10^18
     * N is divisible by K
     */
    public static void solveCodeChefDSTAPLS() {
        Scanner scn = new Scanner(System.in);
        long T = scn.nextLong();
        while (T-- > 0) {
            long N = scn.nextLong();
     ...
     * There are two dishes Chef can make. The first one contains F1 grams of fat and P1 grams of protein, 
     * while the second contains F2 grams of fat and P2 grams of protein.
     ...
     * 
     * Input Format:
     * The first and only line of input contains four space-separated integers F1, P1, F2, and P2 - 
     * the quantities of fat and protein in the first dish and second dish, respectively.
     * 
     * Output Format:
     ...
     * Each character of the output may be in either uppercase or lowercase, 
     * i.e. if the answer is Both, then any of the strings BOTH, both, Both, bOTh, and so on will be accepted.
     * 
     * Constraints:
     * 1 <= F1, P1, F2, P2 <= 100
     */
    public static void solveCodeChefFOODBAL() {
        Scanner scn = new Scanner(System.in);
        long F1 = scn.nextLong();
        long P1 = scn.nextLong();
        long F2 = scn.nextLong();
        long P2 = scn.nextLong();
        long diff1 = Math.abs(F1 - P1);
        long diff2 = Math.abs(F2 - P2);
        if (diff1 == diff2)
            System.out.println("Both");
        else if (diff1 < diff2)
     ...
     */
    public static void solveCodeChefKOL15A() {
        Scanner scn = new Scanner(System.in);
        long T = scn.nextLong();
        while (T-- > 0) {
            String S = scn.next();
